console.log("GeoDiadema 3.2.3 carregado");
...
async function saveFeiraForm(event){
 ...
 try{
  ...
  const result=await response.json();if(!result.sucesso)throw new Error(result.erro||"Não foi possível salvar a feira.");
...
async function saveVivaleiteForm(event){
 ...
 try{
  ...
  const result=await response.json();
  if(!result.sucesso)throw new Error(result.erro||"Não foi possível salvar o ponto.");
