console.log("GeoEventos v1.10 carregado");
...
function fillRouteEventSelect(){
  ...
  allEvents.forEach(e=>{
    ...
    if(id&&!mapEvents.has(id))mapEvents.set(id,e)
  });
  routeEventSelect.innerHTML='<option value="">Selecione uma feira</option>';
  [...mapEvents.values()]
...
function loadSelectedEventRoutes(){
  ...
  getEventRoutes(selectedRouteEvent).forEach(r=>addRouteEditorLayer(r.points));

  if(routeLayerSequence.length){
    map.fitBounds(L.featureGroup(routeLayerSequence).getBounds().pad(.2),{maxZoom:18})
  }else{
  ...
  setRouteEditorStatus(
    routeLayerSequence.length
      ?`${routeLayerSequence.length} rua(s) carregada(s). Você pode redesenhar ou acrescentar outra.`
      :"Clique em “Nova rua” para começar."
  );
}
...
async function saveRoutes(){
  ...
  try{
    const body=new URLSearchParams({
      ...
      percursos:JSON.stringify(percursos)
    });
    ...
    if(!result.sucesso)throw new Error(result.erro||"Não foi possível salvar");
