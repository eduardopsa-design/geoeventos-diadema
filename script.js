console.log("GeoEventos v1.12 carregado");
...
async function locateFairSegment(index,address,button){
  ...
  try{
    ...
    L.marker([startPoint.lat,startPoint.lng],{
      icon:makeFairSearchIcon("A","start"),
      interactive:false
    }).addTo(fairLocationAssistLayer);

    L.marker([endPoint.lat,endPoint.lng],{
      icon:makeFairSearchIcon("B","end"),
      interactive:false
    }).addTo(fairLocationAssistLayer);
    ...
    L.polyline(
      ...
      ],
      {
        ...
        opacity:.75,
        dashArray:"9 7",
        className:"fair-search-guide",
        interactive:false
      }
    ).addTo(fairLocationAssistLayer);

    const guideBounds=L.latLngBounds(
      [startPoint.lat,startPoint.lng],
      [endPoint.lat,endPoint.lng]
    );

    map.fitBounds(guideBounds.pad(.35),{maxZoom:19});
