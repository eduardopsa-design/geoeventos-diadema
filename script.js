console.log("GeoDiadema 3.3.3 carregado");
// Versão 3.3: consolidação da plataforma.
// Prioridade: estabilidade dos cadastros de Eventos, Hortas, Feiras e VIVALEITE.
// Novos módulos devem reutilizar as funções existentes, evitando duplicação de lógica.
...
primaryTileLayer.on("tileerror",()=>{
 tileErrorCount++;
 if(tileErrorCount>=4&&!map.hasLayer(fallbackTileLayer)){
  map.removeLayer(primaryTileLayer);
  fallbackTileLayer.addTo(map);
  statusBox.textContent="Mapa alternativo carregado.";
 }
});

const markersLayer = L.markerClusterGroup();
const hortasLayer = L.markerClusterGroup();
const vivaleiteLayer = L.markerClusterGroup();
...
let allHortas = [];
let allVivaleite = [];
let vivaleiteInternal = [];
let vivaleiteActive = false;
let vivaleiteTodayOnly = false;
...
let feiraFormMode="new";
let saveAndCreateAnotherFeira=false;
let vivaleiteFormMode="new";
let saveAndCreateAnotherVivaleite=false;
let favoriteKeys=new Set(JSON.parse(localStorage.getItem("geodiademaFavorites")||"[]"));
let visitFolderKeys=JSON.parse(localStorage.getItem("geodiademaVisitFolder")||"[]");
...
const exploreVivaleiteToggle = document.getElementById("exploreVivaleiteToggle");
const exploreVivaleiteCount = document.getElementById("exploreVivaleiteCount");
...
const gabineteIncludeVivaleite = document.getElementById("gabineteIncludeVivaleite");
...
const dataCenterVivaleiteCount=document.getElementById("dataCenterVivaleiteCount");
...
const eventValidationSummary=document.getElementById("eventValidationSummary");
...
const newFeiraTypeBtn=document.getElementById("newFeiraTypeBtn");
const feiraFormModal=document.getElementById("feiraFormModal");
const feiraForm=document.getElementById("feiraForm");
const feiraFormTitle=document.getElementById("feiraFormTitle");
const feiraFormSubtitle=document.getElementById("feiraFormSubtitle");
const feiraFormOriginalId=document.getElementById("feiraFormOriginalId");
const closeFeiraFormBtn=document.getElementById("closeFeiraFormBtn");
const cancelFeiraFormBtn=document.getElementById("cancelFeiraFormBtn");
const saveFeiraFormBtn=document.getElementById("saveFeiraFormBtn");
const saveAndNewFeiraFormBtn=document.getElementById("saveAndNewFeiraFormBtn");
const feiraFormStatus=document.getElementById("feiraFormStatus");
const feiraNameInput=document.getElementById("feiraNameInput");
const feiraDayInput=document.getElementById("feiraDayInput");
const feiraStatusInput=document.getElementById("feiraStatusInput");
const feiraStartInput=document.getElementById("feiraStartInput");
const feiraEndInput=document.getElementById("feiraEndInput");
const feiraRegionInput=document.getElementById("feiraRegionInput");
const feiraBairroInput=document.getElementById("feiraBairroInput");
const feiraAddressInput=document.getElementById("feiraAddressInput");
const feiraTrecho1Input=document.getElementById("feiraTrecho1Input");
const feiraTrecho2Input=document.getElementById("feiraTrecho2Input");
const feiraTrecho3Input=document.getElementById("feiraTrecho3Input");
const feiraTrecho4Input=document.getElementById("feiraTrecho4Input");
const feiraLatitudeInput=document.getElementById("feiraLatitudeInput");
const feiraLongitudeInput=document.getElementById("feiraLongitudeInput");
const feiraNotesInput=document.getElementById("feiraNotesInput");
const feiraFormQualityPercent=document.getElementById("feiraFormQualityPercent");
const feiraFormQualityFill=document.getElementById("feiraFormQualityFill");
const feiraFormQualityMessage=document.getElementById("feiraFormQualityMessage");

const newVivaleiteTypeBtn=document.getElementById("newVivaleiteTypeBtn");
const vivaleiteFormModal=document.getElementById("vivaleiteFormModal");
const vivaleiteForm=document.getElementById("vivaleiteForm");
const vivaleiteFormTitle=document.getElementById("vivaleiteFormTitle");
const vivaleiteFormSubtitle=document.getElementById("vivaleiteFormSubtitle");
const vivaleiteOriginalId=document.getElementById("vivaleiteOriginalId");
const closeVivaleiteFormBtn=document.getElementById("closeVivaleiteFormBtn");
const cancelVivaleiteFormBtn=document.getElementById("cancelVivaleiteFormBtn");
const saveVivaleiteBtn=document.getElementById("saveVivaleiteBtn");
const saveAndNewVivaleiteBtn=document.getElementById("saveAndNewVivaleiteBtn");
const vivaleiteFormStatusMessage=document.getElementById("vivaleiteFormStatusMessage");
const vivaleiteDeliveryDayInputs=[...document.querySelectorAll('input[name="vivaleiteDeliveryDay"]')];
const vivaleiteDeliveryStatusInput=document.getElementById("vivaleiteDeliveryStatusInput");
const vivaleiteDeliveryReasonInput=document.getElementById("vivaleiteDeliveryReasonInput");
const vivaleiteNextDeliveryPreview=document.getElementById("vivaleiteNextDeliveryPreview");
const vivaleiteLastUpdatePreview=document.getElementById("vivaleiteLastUpdatePreview");
const vivaleiteNameInput=document.getElementById("vivaleiteNameInput");
const vivaleiteStatusInput=document.getElementById("vivaleiteStatusInput");
const vivaleiteStartInput=document.getElementById("vivaleiteStartInput");
const vivaleiteEndInput=document.getElementById("vivaleiteEndInput");
const vivaleiteRegionInput=document.getElementById("vivaleiteRegionInput");
const vivaleiteBairroInput=document.getElementById("vivaleiteBairroInput");
const vivaleiteAddressInput=document.getElementById("vivaleiteAddressInput");
const vivaleiteLatitudeInput=document.getElementById("vivaleiteLatitudeInput");
const vivaleiteLongitudeInput=document.getElementById("vivaleiteLongitudeInput");
const vivaleitePublicDescriptionInput=document.getElementById("vivaleitePublicDescriptionInput");
const vivaleiteLocalResponsibleInput=document.getElementById("vivaleiteLocalResponsibleInput");
const vivaleiteSubstituteInput=document.getElementById("vivaleiteSubstituteInput");
const vivaleitePhoneInput=document.getElementById("vivaleitePhoneInput");
const vivaleiteEmailInput=document.getElementById("vivaleiteEmailInput");
const vivaleiteChildrenInput=document.getElementById("vivaleiteChildrenInput");
const vivaleiteElderlyInput=document.getElementById("vivaleiteElderlyInput");
const vivaleiteCapacityInput=document.getElementById("vivaleiteCapacityInput");
const vivaleiteVacanciesInput=document.getElementById("vivaleiteVacanciesInput");
const vivaleiteInternalNotesInput=document.getElementById("vivaleiteInternalNotesInput");
const vivaleiteQualityPercent=document.getElementById("vivaleiteQualityPercent");
const vivaleiteQualityFill=document.getElementById("vivaleiteQualityFill");
const vivaleiteQualityMessage=document.getElementById("vivaleiteQualityMessage");
...
const visitFolderBtn=document.getElementById("visitFolderBtn");
const visitFolderPanel=document.getElementById("visitFolderPanel");
const closeVisitFolderBtn=document.getElementById("closeVisitFolderBtn");
const visitFolderBackdrop=document.getElementById("visitFolderBackdrop");
const visitFolderList=document.getElementById("visitFolderList");
const visitFolderCount=document.getElementById("visitFolderCount");
const vivaleiteTodayBtn=document.getElementById("vivaleiteTodayBtn");
const mobileMapHeader=document.getElementById("mobileMapHeader");
const mobileMapStateText=document.getElementById("mobileMapStateText");
const mobileMapInteractBtn=document.getElementById("mobileMapInteractBtn");
const mobileMapExpandBtn=document.getElementById("mobileMapExpandBtn");
const mobileBottomNav=document.getElementById("mobileBottomNav");
const mobileFab=document.getElementById("mobileFab");
const mobileMoreSheet=document.getElementById("mobileMoreSheet");
const closeMobileMoreSheet=document.getElementById("closeMobileMoreSheet");
const mobileSheetBackdrop=document.getElementById("mobileSheetBackdrop");
const vivaleiteInternalDashboard=document.getElementById("vivaleiteInternalDashboard");
const vlActivePoints=document.getElementById("vlActivePoints");
const vlTodayDeliveries=document.getElementById("vlTodayDeliveries");
const vlChildrenTotal=document.getElementById("vlChildrenTotal");
const vlElderlyTotal=document.getElementById("vlElderlyTotal");
const vlBeneficiariesTotal=document.getElementById("vlBeneficiariesTotal");
const vlCapacityTotal=document.getElementById("vlCapacityTotal");
const vlVacanciesTotal=document.getElementById("vlVacanciesTotal");
const clearVisitFolderBtn=document.getElementById("clearVisitFolderBtn");
const sendVisitToGabineteBtn=document.getElementById("sendVisitToGabineteBtn");
...
function isCoordinateInsideDiadema(lat,lng){
 return Number.isFinite(lat)&&Number.isFinite(lng)&&
  lat>=-23.80&&lat<=-23.58&&
  lng>=-46.75&&lng<=-46.50;
}

function normalizeDiademaCoordinates(latValue,lngValue){
 let lat=parseCoordinate(latValue);
 let lng=parseCoordinate(lngValue);

 if(lat===null||lng===null)return null;

 // Corrige automaticamente latitude/longitude invertidas.
 if(!isCoordinateInsideDiadema(lat,lng)&&isCoordinateInsideDiadema(lng,lat)){
  const temporary=lat;
  lat=lng;
  lng=temporary;
 }

 return isCoordinateInsideDiadema(lat,lng)?[lat,lng]:null;
}
...
function getHortaTypeClass(tipo) {
  ...
  if (normalized.includes("educacional")) return "horta-educacional";
...
function getHortaPosition(horta) {
  return normalizeDiademaCoordinates(horta.latitude,horta.longitude);
}
...
function getEventPosition(evento){
 const direct=normalizeDiademaCoordinates(evento.latitude,evento.longitude);
 if(direct)return direct;

 const routes=getEventRoutes(evento);
 const points=routes
  .flatMap(route=>route.points)
  .map(point=>normalizeDiademaCoordinates(point[0],point[1]))
  .filter(Boolean);
...
function updateExplorePanel() {
  ...
  if(exploreVivaleiteCount){
    exploreVivaleiteCount.textContent = String(allVivaleite.length);
  }

  exploreSummaryText.textContent =
    `${allEvents.length} eventos • ` +
    `${agendaCount} compromissos da agenda • ` +
    `${allHortas.length} hortas • ${allVivaleite.length} pontos VIVALEITE`;
}
...
function getGabineteCategoryConfig() {
  return [
    ...
    },
    {
      ...
      label: "VIVALEITE",
      icon: "🥛",
      enabled: gabineteIncludeVivaleite.checked
    }
  ];
}
...
function createGabineteResults() {
  ...
  const feiras = gabineteIncludeFeiras.checked
    ...
    const vivaleite = gabineteIncludeVivaleite.checked
    ? allVivaleite
        .filter(locationMatches)
        .map(item => ({
          ...item,
          kind: "vivaleite",
          displayName: item.nome || "Ponto do VIVALEITE"
        }))
    : [];

  gabineteResults = events.concat(hortas, feiras, vivaleite);
...
function getGabineteKindMeta(kind) {
  ...
  if (kind === "vivaleite") {
    return { label: "VIVALEITE", icon: "🥛", className: "kind-vivaleite" };
  }
...
function sortGabineteItems(items){
  // Mantém exatamente a ordem definida pelo usuário na seção
  // "Ordem do briefing", inclusive após arrastar os itens.
  return Array.isArray(items) ? [...items] : [];
}

function buildGabinetePreview() {
 if(!gabineteSelectedIds || gabineteSelectedIds.size===0){
  window.alert("Selecione pelo menos um item para visualizar a prévia.");
  return;
 }
...
function updateDataCenterIndicators(){
 ...
 dataCenterEventsCount.textContent=e.length;dataCenterHortasCount.textContent=allHortas.length;dataCenterFeirasCount.textContent=getDataCenterFeiras().length;dataCenterVivaleiteCount.textContent=getVivaleiteAdminRecords().length;dataCenterPendingCount.textContent=q.pendingRecords;
...
function getDataCenterModuleItems(){
 ...
 if(dataCenterActiveModule==="vivaleite")return getVivaleiteAdminRecords();
...
function getDataCenterRecordName(x){
 if(dataCenterActiveModule==="hortas")return x.nome||"Horta";
 if(dataCenterActiveModule==="feiras")return x.feiraBase?.feira||x.evento||"Feira Livre";
 if(dataCenterActiveModule==="vivaleite")return x.nome||"Ponto do VIVALEITE";
 return x.evento||"Evento sem nome";
}
 ...
 items.slice(0,250).forEach(item=>{
  ...
  card.innerHTML=`
   <div>
    ...
    ${dataCenterActiveModule==="vivaleite"?`
      <p><strong>Próxima entrega:</strong> ${escapeHtml(formatVivaleiteNextDelivery(item))}</p>
      <p><strong>Situação de hoje:</strong> ${escapeHtml(item.situacaoEntrega||"Não iniciada")}</p>
      <p><strong>Última atualização:</strong> ${escapeHtml(item.ultimaAtualizacao||"-")}</p>
    `:""}
    ${issuesHtml}
   </div>
   <div class="data-center-record-actions">
    <button class="edit-record-btn" type="button">${issues.length?"✏️ Corrigir agora":"✏️ Editar"}</button>
    <button class="duplicate-record-btn" type="button">📄 Duplicar</button>
    <button class="favorite-record-btn ${favoriteKeys.has(getRecordLibraryKey(item,dataCenterActiveModule==="events"?"evento":dataCenterActiveModule.slice(0,-1)))?"favorite-active":""}" type="button">⭐ Favorito</button>
    <button class="visit-record-btn ${visitFolderKeys.includes(getRecordLibraryKey(item,dataCenterActiveModule==="events"?"evento":dataCenterActiveModule.slice(0,-1)))?"visit-active":""}" type="button">📁 Visita</button>
    <button class="map-record-btn" type="button">📍 Ver no mapa</button>
   </div>
  `;
  ...
  const favoriteButton=card.querySelector(".favorite-record-btn");
  const visitButton=card.querySelector(".visit-record-btn");
  const moduleKind=dataCenterActiveModule==="events"?"evento":dataCenterActiveModule.slice(0,-1);

  editButton.addEventListener("click",()=>{
   const firstIssue=issues.length?issues[0]:null;
   if(dataCenterActiveModule==="hortas")openHortaForm("edit",item);
   else if(dataCenterActiveModule==="feiras")openFeiraForm("edit",item);
   else if(dataCenterActiveModule==="vivaleite")openVivaleiteForm("edit",item);
   else openEventForm("edit",item,firstIssue);
  });
  duplicateButton.addEventListener("click",()=>{
   if(dataCenterActiveModule==="hortas")openHortaForm("duplicate",item);
   else if(dataCenterActiveModule==="feiras")openFeiraForm("duplicate",item);
   else if(dataCenterActiveModule==="vivaleite")openVivaleiteForm("duplicate",item);
   else openEventForm("duplicate",item);
  });
  favoriteButton.addEventListener("click",()=>toggleFavorite(item,moduleKind));
  visitButton.addEventListener("click",()=>toggleVisitFolder(item,moduleKind));
  mapButton.addEventListener("click",()=>focusRecordOnMap(item,moduleKind));

 });
}

function refreshDataCenter(){
 populateDataCenterFilters();
 updateDataCenterIndicators();
 renderDataCenterList();
 updateVivaleiteInternalDashboard();
}
...
function resetEventForm(){
  ...
  if(eventValidationSummary){
    eventValidationSummary.hidden=true;
    eventValidationSummary.innerHTML="";
    eventValidationSummary.className="validation-summary";
  }
}
...
function getCanonicalRegionForBairro(bairro){
 if(!bairro)return "";
 const map=buildBairroRegionMap();
 return map.get(normalize(bairro))||"";
}

function findPossibleDuplicateEvent(payload){
 return allEvents.find(item=>{
  if(isFeiraGabineteItem(item))return false;

  const sameName=normalize(item.evento)===normalize(payload.evento);
  const sameDate=String(item.data||"")===String(payload.data||"") ||
   formatDateForInput(item.data)===payload.data;
  const sameStart=String(item.horaInicio||"").slice(0,5)===payload.horaInicio;
  const sameLocal=
   normalize(item.local||item.endereco)===
   normalize(payload.local||payload.endereco);

  const sameId=String(item.id||item.idBase||"")===String(payload.id||"");

  return !sameId && sameName && sameDate && sameStart && sameLocal;
 });
}
...
function validateEventPayload(payload){
 ...
 const start=timeToMinutes(payload.horaInicio);
 const end=timeToMinutes(payload.horaFim);

 if(start!==null&&end!==null&&end<=start){
  errors.push("O horário final deve ser posterior ao horário inicial.");
 }

 const inferredRegion=getCanonicalRegionForBairro(payload.bairro);
 if(inferredRegion&&payload.regiao&&
    normalize(inferredRegion)!==normalize(payload.regiao)){
  errors.push(
   `O bairro ${payload.bairro} pertence à região ${inferredRegion}.`
  );
 }

 if(!payload.latitude||!payload.longitude){
  warnings.push(
   "O evento será salvo sem coordenadas e poderá não aparecer corretamente no mapa."
  );
 }

 const duplicate=findPossibleDuplicateEvent(payload);
 if(duplicate){
  warnings.push(
   `Possível duplicidade com “${duplicate.evento}” em ${duplicate.data}, `+
   `${duplicate.horaInicio||"-"}, no mesmo local.`
  );
 }
...
function renderEventValidation(result){
 if(!eventValidationSummary)return;
 ...
 const items=[
  ...result.errors.map(text=>`❌ ${escapeHtml(text)}`),
  ...result.warnings.map(text=>`⚠️ ${escapeHtml(text)}`)
 ];

 eventValidationSummary.hidden=false;
 eventValidationSummary.className=
  result.errors.length?"validation-summary":"validation-summary warning";
 eventValidationSummary.innerHTML=
  `<strong>${result.errors.length?"Corrija antes de salvar":"Confirme antes de continuar"}</strong>`+
...
async function saveEventForm(event){
  ...
  renderEventValidation(validation);

  if(validation.errors.length){
    eventFormStatus.textContent="Existem informações que precisam ser corrigidas.";
    return;
  }

  if(validation.warnings.length){
    const confirmed=window.confirm(
      "Foram encontrados avisos no cadastro:\n\n"+
  ...
  try{
    ...
    if(!result.sucesso){
      throw new Error(result.erro||"Não foi possível salvar o evento. Verifique os campos e tente novamente.");
    }
...
async function saveHortaForm(event){
 ...
 try{
  ...
  if(!result.sucesso){
   throw new Error(result.erro||"Não foi possível salvar a horta. Verifique os campos e tente novamente.");
  }
...
function getRecordLibraryKey(item,moduleName=""){
 const kind=moduleName||(
  item.origemRegistro==="Hortas_Base"?"horta":
  item.origemRegistro==="Vivaleite_Publico"||item.origemRegistro==="Vivaleite_Interno"?"vivaleite":
  isFeiraGabineteItem(item)?"feira":"evento"
 );
 const id=item.idBase||item.id||item.nome||item.evento;
 return `${kind}:${id}`;
}

function getAllLibraryRecords(){
 const events=allEvents.filter(item=>!isFeiraGabineteItem(item)).map(item=>({item,kind:"evento"}));
 const fairs=getDataCenterFeiras().map(item=>({item,kind:"feira"}));
 const gardens=allHortas.map(item=>({item,kind:"horta"}));
 const milk=allVivaleite.map(item=>({item,kind:"vivaleite"}));
 return [...events,...fairs,...gardens,...milk];
}

function findLibraryRecord(key){
 return getAllLibraryRecords().find(entry=>getRecordLibraryKey(entry.item,entry.kind)===key)||null;
}

function persistLibraryState(){
 localStorage.setItem("geodiademaFavorites",JSON.stringify([...favoriteKeys]));
 localStorage.setItem("geodiademaVisitFolder",JSON.stringify(visitFolderKeys));
 visitFolderCount.textContent=String(visitFolderKeys.length);
}

function toggleFavorite(item,kind){
 const key=getRecordLibraryKey(item,kind);
 if(favoriteKeys.has(key))favoriteKeys.delete(key);else favoriteKeys.add(key);
 persistLibraryState();renderDataCenterList();renderFavoritesPanel();
}

function toggleVisitFolder(item,kind){
 const key=getRecordLibraryKey(item,kind);
 if(visitFolderKeys.includes(key)){
  visitFolderKeys=visitFolderKeys.filter(value=>value!==key);
 }else{
  visitFolderKeys.push(key);
 }
 persistLibraryState();renderDataCenterList();renderVisitFolderPanel();
}

function getLibraryDisplay(entry){
 const {item,kind}=entry;
 const name=kind==="horta"?(item.nome||"Horta"):
  kind==="feira"?(item.feiraBase?.feira||item.evento||"Feira Livre"):
  (item.evento||"Evento");
 return{
  name,
  icon:kind==="horta"?"🌱":kind==="feira"?"🛒":kind==="vivaleite"?"🥛":"📅",
  territory:[item.bairro,getItemEffectiveRegion(item)].filter(Boolean).join(" / "),
  place:item.local||item.endereco||item.feiraBase?.trechos?.join(" • ")||"-"
 };
}

function focusRecordOnMap(item,kind){
 closeDataCenterPanel();closeFavoritesPanel();closeVisitFolderPanel();
 if(kind==="horta"){
  hortasActive=true;updateHortasButton();renderMap();
  const pos=getHortaPosition(item);
  if(pos)map.setView(pos,17);
 }else if(kind==="vivaleite"){
  vivaleiteActive=true;renderMap();
  const pos=getVivaleitePosition(item);
  if(pos)map.setView(pos,17);
 }else{
  eventsActive=true;
  searchInput.value=kind==="feira"
   ?(item.feiraBase?.feira||item.evento||"")
   :(item.evento||"");
  renderMap();
...
function renderLibraryList(target,keys,mode){
 target.innerHTML="";
 const entries=keys.map(findLibraryRecord).filter(Boolean);
 if(!entries.length){
  target.innerHTML='<div class="gabinete-empty">Nenhum item adicionado.</div>';
  return;
 }
 entries.forEach(entry=>{
  const display=getLibraryDisplay(entry);
  const key=getRecordLibraryKey(entry.item,entry.kind);
  const card=document.createElement("article");
  card.className="mini-library-item";
  card.innerHTML=`
   <div><h3>${display.icon} ${escapeHtml(display.name)}</h3>
   <p>${escapeHtml(display.territory||"-")}</p><p>${escapeHtml(display.place)}</p></div>
   <div class="mini-library-actions">
    <button class="library-map-btn">📍 Mapa</button>
    <button class="library-remove-btn">Remover</button>
   </div>`;
  card.querySelector(".library-map-btn").addEventListener("click",()=>focusRecordOnMap(entry.item,entry.kind));
  card.querySelector(".library-remove-btn").addEventListener("click",()=>{
   if(mode==="favorites")favoriteKeys.delete(key);
   else visitFolderKeys=visitFolderKeys.filter(value=>value!==key);
   persistLibraryState();
   mode==="favorites"?renderFavoritesPanel():renderVisitFolderPanel();
   renderDataCenterList();
  });
...
function renderFavoritesPanel(){renderLibraryList(favoritesList,[...favoriteKeys],"favorites")}
function renderVisitFolderPanel(){renderLibraryList(visitFolderList,visitFolderKeys,"visit")}

function openFavoritesPanel(){
 renderFavoritesPanel();favoritesPanel.classList.add("open");
 favoritesPanel.setAttribute("aria-hidden","false");favoritesBackdrop.hidden=false;
}
function closeFavoritesPanel(){favoritesPanel.classList.remove("open");favoritesPanel.setAttribute("aria-hidden","true");favoritesBackdrop.hidden=true}
function openVisitFolderPanel(){renderVisitFolderPanel();visitFolderPanel.classList.add("open");visitFolderPanel.setAttribute("aria-hidden","false");visitFolderBackdrop.hidden=false}
function closeVisitFolderPanel(){visitFolderPanel.classList.remove("open");visitFolderPanel.setAttribute("aria-hidden","true");visitFolderBackdrop.hidden=true}

function sendVisitFolderToGabinete(){
 const entries=visitFolderKeys.map(findLibraryRecord).filter(Boolean);
 if(!entries.length){window.alert("A Pasta da Visita está vazia.");return}
 closeVisitFolderPanel();openGabinetePanel();
 gabineteResults=entries.map(entry=>({
  ...entry.item,
  kind:entry.kind,
  displayName:getLibraryDisplay(entry).name
 }));
 gabineteSelectedIds=new Set(gabineteResults.map(gabineteItemId));
 gabineteSelectedOrder=gabineteResults.map(gabineteItemId);
 gabineteQueryStep.hidden=true;gabineteResultsStep.hidden=false;gabineteSelectionBar.hidden=false;
 gabineteResultTitle.textContent="Pasta da Visita";
 gabineteResultSubtitle.textContent=`${gabineteResults.length} item(ns) selecionado(s)`;
 renderGabineteCategoryResults();updateGabineteSelection();
}

function fillFeiraFormSuggestions(){
 const regions=dedupeCanonicalTerritories([...allEvents,...allHortas].map(item=>getItemEffectiveRegion(item)));
 const bairros=dedupeCanonicalTerritories([...allEvents,...allHortas].map(item=>item.bairro));
 feiraRegionInput.innerHTML='<option value="">Selecione</option>'+regions.map(v=>`<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join("");
 feiraBairroInput.innerHTML='<option value="">Selecione</option>'+bairros.map(v=>`<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join("");
}
function inferFeiraRegion(){
 const inferred=buildBairroRegionMap().get(normalize(feiraBairroInput.value))||"";
 if(inferred)feiraRegionInput.value=inferred;
}
function resetFeiraForm(){feiraForm.reset();feiraFormOriginalId.value="";feiraStatusInput.value="Ativa";feiraFormStatus.textContent="";feiraFormStatus.className="event-form-status"}
function fillFeiraForm(record){
 const base=record.feiraBase||record;
 feiraFormOriginalId.value=record.idBase||base.id||record.id||"";
 feiraNameInput.value=base.feira||record.evento||"";
 feiraDayInput.value=base.dia||"";
 feiraStatusInput.value=base.status||record.status||"Ativa";
 feiraStartInput.value=base.horarioInicio||record.horaInicio||"";
 feiraEndInput.value=base.horarioFim||record.horaFim||"";
 feiraRegionInput.value=base.regiao||getItemEffectiveRegion(record)||"";
 feiraBairroInput.value=record.bairro||base.bairro||"";
 feiraAddressInput.value=record.endereco||base.endereco||"";
 const trechos=base.trechos||[];
 feiraTrecho1Input.value=trechos[0]||"";
 feiraTrecho2Input.value=trechos[1]||"";
 feiraTrecho3Input.value=trechos[2]||"";
 feiraTrecho4Input.value=trechos[3]||"";
 feiraLatitudeInput.value=record.latitude||base.latitude||"";
 feiraLongitudeInput.value=record.longitude||base.longitude||"";
 feiraNotesInput.value=base.observacoes||record.observacoes||"";
}
function openFeiraForm(mode,record=null){
 feiraFormMode=mode;resetFeiraForm();fillFeiraFormSuggestions();if(record)fillFeiraForm(record);
 if(mode==="edit"){feiraFormTitle.textContent="✏️ Editar feira";feiraFormSubtitle.textContent="Altere os dados e salve."}
 else if(mode==="duplicate"){feiraFormTitle.textContent="📄 Duplicar feira";feiraFormSubtitle.textContent="Será criado um novo ID.";feiraFormOriginalId.value=""}
 else{feiraFormTitle.textContent="🛒 Nova feira";feiraFormSubtitle.textContent="Preencha os dados da feira livre."}
 updateFeiraFormQuality();feiraFormModal.classList.add("open");feiraFormModal.setAttribute("aria-hidden","false")
}
function closeFeiraForm(){feiraFormModal.classList.remove("open");feiraFormModal.setAttribute("aria-hidden","true")}
function getFeiraMissingFields(){
 const m=[];if(!feiraNameInput.value.trim())m.push("nome");if(!feiraDayInput.value)m.push("dia");
 if(!feiraStartInput.value)m.push("horário inicial");if(!feiraEndInput.value)m.push("horário final");
 if(!feiraBairroInput.value)m.push("bairro");if(!feiraAddressInput.value.trim())m.push("endereço");return m
}
function updateFeiraFormQuality(){
 const vals=[feiraNameInput.value.trim(),feiraDayInput.value,feiraStatusInput.value,feiraStartInput.value,feiraEndInput.value,feiraRegionInput.value,feiraBairroInput.value,feiraAddressInput.value.trim(),feiraLatitudeInput.value.trim(),feiraLongitudeInput.value.trim()];
 const percent=Math.round(vals.filter(Boolean).length/vals.length*100);feiraFormQualityPercent.textContent=percent+"%";feiraFormQualityFill.style.width=percent+"%";
 const m=getFeiraMissingFields();feiraFormQualityMessage.textContent=m.length?"Falta preencher: "+m.join(", ")+".":"Todos os campos obrigatórios estão preenchidos."
}
function getFeiraPayload(){
 return{acao:feiraFormMode==="edit"?"updateFeira":"createFeira",codigo:"3648",id:feiraFormOriginalId.value.trim(),
 feira:feiraNameInput.value.trim(),dia:feiraDayInput.value,status:feiraStatusInput.value,
 horarioInicio:feiraStartInput.value,horarioFim:feiraEndInput.value,regiao:canonicalTerritoryName(feiraRegionInput.value),
 bairro:canonicalTerritoryName(feiraBairroInput.value),endereco:feiraAddressInput.value.trim(),
 trecho1:feiraTrecho1Input.value.trim(),trecho2:feiraTrecho2Input.value.trim(),trecho3:feiraTrecho3Input.value.trim(),trecho4:feiraTrecho4Input.value.trim(),
 latitude:feiraLatitudeInput.value.trim(),longitude:feiraLongitudeInput.value.trim(),observacoes:feiraNotesInput.value.trim()}
}
async function saveFeiraForm(event){
 event.preventDefault();const missing=getFeiraMissingFields();if(missing.length){feiraFormStatus.textContent="Preencha os campos obrigatórios.";return}
 if(timeToMinutes(feiraEndInput.value)<=timeToMinutes(feiraStartInput.value)){feiraFormStatus.textContent="O horário final deve ser posterior ao inicial.";return}
 saveFeiraFormBtn.disabled=true;saveFeiraFormBtn.textContent="Salvando...";
 try{
  const params=new URLSearchParams();Object.entries(getFeiraPayload()).forEach(([k,v])=>params.set(k,String(v??"")));
  const response=await fetch(API_URL,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},body:params.toString()});
  const result=await response.json();if(!result.sucesso)throw new Error(result.erro||"Não foi possível salvar a feira. Verifique os campos e tente novamente.");
  feiraFormStatus.textContent=feiraFormMode==="edit"?"Feira atualizada com sucesso.":"Feira criada com sucesso.";feiraFormStatus.className="event-form-status success";
  await loadEvents();refreshDataCenter();
  if(saveAndCreateAnotherFeira){saveAndCreateAnotherFeira=false;resetFeiraForm();fillFeiraFormSuggestions();updateFeiraFormQuality()}
  else setTimeout(closeFeiraForm,700);
 }catch(error){feiraFormStatus.textContent=error.message;feiraFormStatus.className="event-form-status"}
 finally{saveFeiraFormBtn.disabled=false;saveFeiraFormBtn.textContent="💾 Salvar feira"}
}



function getSelectedVivaleiteDays(){
 return vivaleiteDeliveryDayInputs
  .filter(input=>input.checked)
  .map(input=>input.value);
}

function normalizeWeekdayName(value){
 return normalize(value).replace("feira","");
}
...
function weekdayIndexFromName(value){
 ...
 return map[normalizeWeekdayName(value)] ?? null;
}

function getNextVivaleiteDelivery(item,fromDate=new Date()){
 const days=String(item.diasEntrega||"")
  .split(",")
  .map(value=>value.trim())
  .filter(Boolean);

 const indices=days
  .map(weekdayIndexFromName)
  .filter(value=>value!==null);
 ...
 indices.forEach(dayIndex=>{
  ...
  let diff=(dayIndex-candidate.getDay()+7)%7;
  candidate.setDate(candidate.getDate()+diff);

  const [hour,minute]=String(item.horaInicio||"00:00")
   .split(":")
   .map(Number);
...
function formatVivaleiteNextDelivery(item,fromDate=new Date()){
 const next=getNextVivaleiteDelivery(item,fromDate);
 if(!next)return "Dias de entrega não definidos";
 ...
 if(diff===0)return `🟢 Entrega hoje às ${time}`;
 if(diff===1)return `🟡 Entrega amanhã às ${time}`;

 return `🔵 Próxima entrega: ${next.toLocaleDateString("pt-BR",{
  weekday:"long",day:"2-digit",month:"2-digit"
 })} às ${time} • faltam ${diff} dias`;
}

function vivaleiteDeliversToday(item,date=new Date()){
 const today=date.getDay();
 return String(item.diasEntrega||"")
  .split(",")
  .map(value=>weekdayIndexFromName(value.trim()))
  .includes(today);
}

function updateVivaleiteNextDeliveryPreview(){
 const item={
  diasEntrega:getSelectedVivaleiteDays().join(", "),
  horaInicio:vivaleiteStartInput.value
 };
 vivaleiteNextDeliveryPreview.textContent=formatVivaleiteNextDelivery(item);
}

function updateVivaleiteInternalDashboard(){
 if(!vivaleiteInternalDashboard)return;

 const records=getVivaleiteAdminRecords();
 const sum=field=>records.reduce(
  (total,item)=>total+(Number(item[field])||0),0
 );

 if(vlActivePoints){
  vlActivePoints.textContent=records.filter(
   item=>normalize(item.status)==="ativo"
  ).length;
 }
 if(vlTodayDeliveries){
  vlTodayDeliveries.textContent=records.filter(vivaleiteDeliversToday).length;
 }
 if(vlChildrenTotal)vlChildrenTotal.textContent=sum("criancas").toLocaleString("pt-BR");
 if(vlElderlyTotal)vlElderlyTotal.textContent=sum("idosos").toLocaleString("pt-BR");
 if(vlBeneficiariesTotal){
  vlBeneficiariesTotal.textContent=
   (sum("criancas")+sum("idosos")).toLocaleString("pt-BR");
 }
 if(vlCapacityTotal)vlCapacityTotal.textContent=sum("capacidade").toLocaleString("pt-BR");
 if(vlVacanciesTotal)vlVacanciesTotal.textContent=sum("vagas").toLocaleString("pt-BR");

 vivaleiteInternalDashboard.hidden=dataCenterActiveModule!=="vivaleite";
}

function createVivaleiteIcon(){
 return L.divIcon({
  className:"",
  html:'<div class="vivaleite-pin"><span>🥛</span></div>',
  iconSize:[38,38],iconAnchor:[19,38],popupAnchor:[0,-35]
 });
}

function getVivaleitePosition(item){
 return normalizeDiademaCoordinates(item.latitude,item.longitude);
}

function buildVivaleitePublicPopup(item){
 const destination=getVivaleitePosition(item)
  ?getVivaleitePosition(item).join(",")
  :[item.endereco,item.bairro,"Diadema - SP"].filter(Boolean).join(", ");

 return `
  <div class="event-popup">
   <div class="vivaleite-public-badge">🥛 VIVALEITE</div>
   <h3 class="popup-title">${escapeHtml(item.nome||"Ponto do VIVALEITE")}</h3>
   <div class="popup-row"><span>📍</span><div><span class="popup-label">Endereço:</span> ${escapeHtml(item.endereco||"-")}</div></div>
   <div class="popup-row"><span>🏘️</span><div><span class="popup-label">Bairro/Região:</span> ${escapeHtml(item.bairro||"-")} / ${escapeHtml(item.regiao||"-")}</div></div>
   <div class="popup-row"><span>📅</span><div><span class="popup-label">Entrega:</span> ${escapeHtml(item.diasEntrega||"-")}</div></div>
   <div class="popup-row"><span>🕒</span><div><span class="popup-label">Horário:</span> ${escapeHtml(item.horaInicio||"-")} às ${escapeHtml(item.horaFim||"-")}</div></div>
   <div class="popup-row"><span>✅</span><div><span class="popup-label">Status:</span> ${escapeHtml(item.status||"-")}</div></div>
   <div class="${vivaleiteDeliversToday(item)?"vivaleite-delivery-today":"vivaleite-delivery-next"}">
    ${escapeHtml(formatVivaleiteNextDelivery(item))}
   </div>
   ${item.descricaoPublica?`<div class="popup-row"><span>ℹ️</span><div>${escapeHtml(item.descricaoPublica)}</div></div>`:""}
   <div class="popup-actions">
    <a class="como-chegar-btn" target="_blank" rel="noopener noreferrer"
       href="https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(destination)}&travelmode=driving">🚗 Como chegar</a>
   </div>
  </div>`;
}

function renderVivaleiteLayer(){
 vivaleiteLayer.clearLayers();
 if(!vivaleiteActive){
  if(map.hasLayer(vivaleiteLayer))map.removeLayer(vivaleiteLayer);
  return [];
 }
 const markers=[];
 allVivaleite
  .filter(item=>!vivaleiteTodayOnly||vivaleiteDeliversToday(item))
  .forEach(item=>{
  const pos=getVivaleitePosition(item);if(!pos)return;
  const marker=L.marker(pos,{icon:createVivaleiteIcon()})
   .bindPopup(buildVivaleitePublicPopup(item),{maxWidth:330});
  vivaleiteLayer.addLayer(marker);markers.push(marker);
 });
 if(!map.hasLayer(vivaleiteLayer))map.addLayer(vivaleiteLayer);
 return markers;
}

async function loadVivaleiteInternal(){
 if(!dataCenterUnlocked)return;
 const params=new URLSearchParams({acao:"listVivaleiteInternal",codigo:"3648"});
 const response=await fetch(API_URL,{
  method:"POST",
  headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},
  body:params.toString()
 ...
 const result=await response.json();
 if(!result.sucesso)throw new Error(result.erro||"Não foi possível carregar os dados internos.");
 vivaleiteInternal=Array.isArray(result.registros)?result.registros:[];
}

function getVivaleiteAdminRecords(){
 return vivaleiteInternal.length?vivaleiteInternal:allVivaleite;
}

function fillVivaleiteSuggestions(){
 const regions=dedupeCanonicalTerritories([...allEvents,...allHortas,...allVivaleite].map(getItemEffectiveRegion));
 const bairros=dedupeCanonicalTerritories([...allEvents,...allHortas,...allVivaleite].map(item=>item.bairro));
 vivaleiteRegionInput.innerHTML='<option value="">Selecione</option>'+
  regions.map(value=>`<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`).join("");
 vivaleiteBairroInput.innerHTML='<option value="">Selecione</option>'+
  bairros.map(value=>`<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`).join("");
}

function inferVivaleiteRegion(){
 const inferred=buildBairroRegionMap().get(normalize(vivaleiteBairroInput.value))||"";
 if(inferred)vivaleiteRegionInput.value=inferred;
}

function resetVivaleiteForm(){
 vivaleiteForm.reset();
 vivaleiteOriginalId.value="";
 vivaleiteStatusInput.value="Ativo";
 vivaleiteDeliveryStatusInput.value="Não iniciada";
 vivaleiteDeliveryReasonInput.value="";
 vivaleiteDeliveryDayInputs.forEach(input=>input.checked=false);
 vivaleiteLastUpdatePreview.textContent="Última atualização ainda não disponível.";
 vivaleiteFormStatusMessage.textContent="";
 vivaleiteFormStatusMessage.className="event-form-status";
}

function fillVivaleiteForm(item){
 vivaleiteOriginalId.value=item.id||"";
 vivaleiteNameInput.value=item.nome||"";
 vivaleiteStatusInput.value=item.status||"Ativo";
   const selectedDays=String(item.diasEntrega||"")
   .split(",")
   .map(value=>value.trim())
   .filter(Boolean);
 vivaleiteDeliveryDayInputs.forEach(input=>{
  input.checked=selectedDays.includes(input.value);
 });
 vivaleiteStartInput.value=item.horaInicio||"";
 vivaleiteEndInput.value=item.horaFim||"";
 vivaleiteRegionInput.value=item.regiao||"";
 vivaleiteBairroInput.value=item.bairro||"";
 vivaleiteAddressInput.value=item.endereco||"";
 vivaleiteLatitudeInput.value=item.latitude||"";
 vivaleiteLongitudeInput.value=item.longitude||"";
 vivaleitePublicDescriptionInput.value=item.descricaoPublica||"";
 vivaleiteLocalResponsibleInput.value=item.responsavelLocal||"";
 vivaleiteSubstituteInput.value=item.responsavelSubstituto||"";
 vivaleitePhoneInput.value=item.telefone||"";
 vivaleiteEmailInput.value=item.email||"";
 vivaleiteChildrenInput.value=item.criancas||"";
 vivaleiteElderlyInput.value=item.idosos||"";
 vivaleiteCapacityInput.value=item.capacidade||"";
 vivaleiteVacanciesInput.value=item.vagas||"";
 vivaleiteInternalNotesInput.value=item.observacoesInternas||"";
 vivaleiteDeliveryStatusInput.value=item.situacaoEntrega||"Não iniciada";
 vivaleiteDeliveryReasonInput.value=item.justificativaEntrega||"";
 vivaleiteLastUpdatePreview.textContent=item.ultimaAtualizacao
  ?`Última atualização: ${item.ultimaAtualizacao}`
  :"Última atualização ainda não disponível.";
}

function openVivaleiteForm(mode,item=null){
 vivaleiteFormMode=mode;resetVivaleiteForm();fillVivaleiteSuggestions();
 if(item)fillVivaleiteForm(item);
 if(mode==="edit"){
  vivaleiteFormTitle.textContent="✏️ Editar ponto do VIVALEITE";
  vivaleiteFormSubtitle.textContent="Dados internos permanecem restritos.";
 }else if(mode==="duplicate"){
  vivaleiteFormTitle.textContent="📄 Duplicar ponto do VIVALEITE";
  vivaleiteFormSubtitle.textContent="Será criado um novo ID.";
  vivaleiteOriginalId.value="";
 }else{
  vivaleiteFormTitle.textContent="🥛 Novo ponto do VIVALEITE";
  vivaleiteFormSubtitle.textContent="Os dados internos não aparecem para usuários comuns.";
 }
 updateVivaleiteQuality();
 vivaleiteFormModal.classList.add("open");
 vivaleiteFormModal.setAttribute("aria-hidden","false");
}

function closeVivaleiteForm(){
 vivaleiteFormModal.classList.remove("open");
 vivaleiteFormModal.setAttribute("aria-hidden","true");
}

function getVivaleiteMissing(){
 const missing=[];
 if(!vivaleiteNameInput.value.trim())missing.push("nome");
 if(!vivaleiteStatusInput.value)missing.push("status");
 if(!vivaleiteDeliveryDayInputs.some(input=>input.checked))missing.push("dias de entrega");
 if(!vivaleiteStartInput.value)missing.push("horário inicial");
 if(!vivaleiteEndInput.value)missing.push("horário final");
 if(!vivaleiteBairroInput.value)missing.push("bairro");
 if(!vivaleiteAddressInput.value.trim())missing.push("endereço");
 return missing;
}

function updateVivaleiteQuality(){
 const vals=[
  vivaleiteNameInput.value.trim(),vivaleiteStatusInput.value,
  getSelectedVivaleiteDays().length?getSelectedVivaleiteDays().join(", "):"",vivaleiteStartInput.value,
  vivaleiteEndInput.value,vivaleiteRegionInput.value,
  vivaleiteBairroInput.value,vivaleiteAddressInput.value.trim(),
  vivaleiteLatitudeInput.value.trim(),vivaleiteLongitudeInput.value.trim()
 ];
 const percent=Math.round(vals.filter(Boolean).length/vals.length*100);
 vivaleiteQualityPercent.textContent=percent+"%";
 vivaleiteQualityFill.style.width=percent+"%";
 const missing=getVivaleiteMissing();
 vivaleiteQualityMessage.textContent=missing.length
  ?"Falta preencher: "+missing.join(", ")+"."
  :"Todos os campos obrigatórios estão preenchidos.";
 updateVivaleiteNextDeliveryPreview();
}

function getVivaleitePayload(){
 return{
  acao:vivaleiteFormMode==="edit"?"updateVivaleite":"createVivaleite",
  codigo:"3648",
  id:vivaleiteOriginalId.value.trim(),
  nome:vivaleiteNameInput.value.trim(),
  status:vivaleiteStatusInput.value,
  diasEntrega:getSelectedVivaleiteDays().join(", "),
  horaInicio:vivaleiteStartInput.value,
  horaFim:vivaleiteEndInput.value,
  regiao:canonicalTerritoryName(vivaleiteRegionInput.value),
  bairro:canonicalTerritoryName(vivaleiteBairroInput.value),
  endereco:vivaleiteAddressInput.value.trim(),
  latitude:vivaleiteLatitudeInput.value.trim(),
  longitude:vivaleiteLongitudeInput.value.trim(),
  descricaoPublica:vivaleitePublicDescriptionInput.value.trim(),
  responsavelLocal:vivaleiteLocalResponsibleInput.value.trim(),
  responsavelSubstituto:vivaleiteSubstituteInput.value.trim(),
  telefone:vivaleitePhoneInput.value.trim(),
  email:vivaleiteEmailInput.value.trim(),
  criancas:vivaleiteChildrenInput.value.trim(),
  idosos:vivaleiteElderlyInput.value.trim(),
  capacidade:vivaleiteCapacityInput.value.trim(),
  vagas:vivaleiteVacanciesInput.value.trim(),
  situacaoEntrega:vivaleiteDeliveryStatusInput.value,
  justificativaEntrega:vivaleiteDeliveryReasonInput.value.trim(),
  observacoesInternas:vivaleiteInternalNotesInput.value.trim()
 };
}

async function saveVivaleiteForm(event){
 event.preventDefault();
 const missing=getVivaleiteMissing();
 if(missing.length){
  vivaleiteFormStatusMessage.textContent="Preencha os campos obrigatórios.";
  return;
 }
 if(timeToMinutes(vivaleiteEndInput.value)<=timeToMinutes(vivaleiteStartInput.value)){
  vivaleiteFormStatusMessage.textContent="O horário final deve ser posterior ao inicial.";
  return;
 }
 saveVivaleiteBtn.disabled=true;saveVivaleiteBtn.textContent="Salvando...";
 try{
  const params=new URLSearchParams();
  Object.entries(getVivaleitePayload()).forEach(([key,value])=>params.set(key,String(value??"")));
  const response=await fetch(API_URL,{
   method:"POST",
   headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},
   body:params.toString()
  ...
  const result=await response.json();
  if(!result.sucesso)throw new Error(result.erro||"Não foi possível salvar o ponto do VIVALEITE. Verifique os campos e tente novamente.");
  vivaleiteFormStatusMessage.textContent=vivaleiteFormMode==="edit"
   ?"Ponto atualizado com sucesso.":"Ponto criado com sucesso.";
  vivaleiteFormStatusMessage.className="event-form-status success";
  await loadEvents();
  await loadVivaleiteInternal();
  refreshDataCenter();
  if(saveAndCreateAnotherVivaleite){
   saveAndCreateAnotherVivaleite=false;resetVivaleiteForm();fillVivaleiteSuggestions();updateVivaleiteQuality();
  }else setTimeout(closeVivaleiteForm,700);
 }catch(error){
  vivaleiteFormStatusMessage.textContent=error.message;
  vivaleiteFormStatusMessage.className="event-form-status";
 }finally{
  saveVivaleiteBtn.disabled=false;saveVivaleiteBtn.textContent="💾 Salvar ponto";
 }
}


function isMobileLayout(){return window.matchMedia("(max-width: 768px)").matches}
function applyMobileMapState(){
 if(!isMobileLayout()){
  document.body.classList.remove("mobile-map-locked","mobile-map-active","mobile-map-expanded");
  setTimeout(()=>map.invalidateSize(),80);return
 }
 document.body.classList.toggle("mobile-map-active",mobileMapInteractionEnabled||mobileMapExpanded);
 document.body.classList.toggle("mobile-map-locked",!mobileMapInteractionEnabled&&!mobileMapExpanded);
 document.body.classList.toggle("mobile-map-expanded",mobileMapExpanded);
 if(mobileMapInteractBtn){
  mobileMapInteractBtn.textContent=mobileMapInteractionEnabled||mobileMapExpanded?"Desativar":"Ativar";
 }
 if(mobileMapExpandBtn){
  mobileMapExpandBtn.textContent=mobileMapExpanded?"Fechar mapa":"Expandir";
 }
 if(mobileMapStateText){
  mobileMapStateText.textContent=mobileMapExpanded
   ?"Mapa em tela ampliada"
   :mobileMapInteractionEnabled
    ?"Mapa ativo: arraste e use o zoom"
    :"Toque em “Ativar” para movimentar o mapa";
 }
 setTimeout(()=>map.invalidateSize(),100)
}
function toggleMobileMapInteraction(){
 if(mobileMapExpanded){mobileMapExpanded=false;mobileMapInteractionEnabled=false}
 else mobileMapInteractionEnabled=!mobileMapInteractionEnabled;
 applyMobileMapState()
}
function toggleMobileMapExpansion(){
 mobileMapExpanded=!mobileMapExpanded;mobileMapInteractionEnabled=mobileMapExpanded;applyMobileMapState()
}
function openMobileMoreSheet(){mobileMoreSheet.classList.add("open");mobileMoreSheet.setAttribute("aria-hidden","false");mobileSheetBackdrop.hidden=false}
function closeMobileMoreSheetPanel(){mobileMoreSheet.classList.remove("open");mobileMoreSheet.setAttribute("aria-hidden","true");mobileSheetBackdrop.hidden=true}
function goToMobileMap(){closeMobileMoreSheetPanel();window.scrollTo({top:0,behavior:"smooth"})}
function focusMobileSearch(){closeMobileMoreSheetPanel();searchInput.focus();searchInput.scrollIntoView({behavior:"smooth",block:"start"})}
function useCurrentLocation(){
 closeMobileMoreSheetPanel();
 if(!navigator.geolocation){window.alert("A localização não está disponível neste navegador.");return}
 navigator.geolocation.getCurrentPosition(position=>{
  const lat=position.coords.latitude,lng=position.coords.longitude;
  mobileMapInteractionEnabled=true;applyMobileMapState();map.setView([lat,lng],16);
  L.circleMarker([lat,lng],{radius:8,weight:3,color:"#fff",fillColor:"#0b4b88",fillOpacity:1}).addTo(map).bindPopup("📍 Sua localização").openPopup();
  statusBox.textContent="Mapa centralizado na sua localização."
 },()=>window.alert("Não foi possível acessar sua localização. Verifique a permissão do navegador."),{enableHighAccuracy:true,timeout:10000,maximumAge:60000})
}
function initializeMobileExperience(){
 if(!isMobileLayout()){applyMobileMapState();return}
 mobileMapInteractionEnabled=false;mobileMapExpanded=false;applyMobileMapState()
}
...
function renderMap() {
  ...
  const vivaleiteMarkers = renderVivaleiteLayer();
  ...
  const fitMarkers = validMarkers.concat(hortaMarkers, vivaleiteMarkers);

  const invalidEventCoordinates=events.filter(item=>{
   const hasAnyCoordinate=String(item.latitude||"").trim()||String(item.longitude||"").trim();
   return hasAnyCoordinate&&!getEventPosition(item);
  }).length;

  if(invalidEventCoordinates>0){
   statusBox.textContent+=` · ${invalidEventCoordinates} coordenada(s) inválida(s) ignorada(s)`;
  }
  ...
  if (fitMarkers.length > 0) {
    ...
    if(bounds.isValid()){
      ...
      map.setView([-23.6865,-46.6234],13);
    }
...
async function loadEvents() {
  try {
    ...
    allVivaleite = data.filter(
      item => item && item.origemRegistro === "Vivaleite_Publico"
    );

    allEvents = data.filter(
      item => !item ||
        (item.origemRegistro !== "Hortas_Base" &&
         item.origemRegistro !== "Vivaleite_Publico")
    );
    ...
    allVivaleite = allVivaleite.map(item => ({
      ...item,
      bairro: canonicalTerritoryName(item.bairro),
      regiao: canonicalTerritoryName(item.regiao)
    }));
    ...
    console.error("Falha no carregamento do GeoDiadema:", error);
    statusBox.textContent =
...
eventBairroInput.addEventListener("change",()=>{
 ...
 updateEventFormQuality();
 const payload=getEventFormPayload();
 renderEventValidation(validateEventPayload(payload));
});
...
newFeiraTypeBtn.addEventListener("click",()=>{closeRecordTypeModal();openFeiraForm("new")});
newVivaleiteTypeBtn.addEventListener("click",()=>{closeRecordTypeModal();openVivaleiteForm("new")});
closeVivaleiteFormBtn.addEventListener("click",closeVivaleiteForm);
cancelVivaleiteFormBtn.addEventListener("click",closeVivaleiteForm);
vivaleiteFormModal.addEventListener("click",event=>{if(event.target===vivaleiteFormModal)closeVivaleiteForm()});
vivaleiteForm.addEventListener("submit",event=>{
 if(event.submitter===saveVivaleiteBtn)saveAndCreateAnotherVivaleite=false;
 saveVivaleiteForm(event);
});
saveAndNewVivaleiteBtn.addEventListener("click",()=>{
 saveAndCreateAnotherVivaleite=true;
 vivaleiteForm.requestSubmit();
});
vivaleiteBairroInput.addEventListener("change",()=>{
 inferVivaleiteRegion();updateVivaleiteQuality();
});
vivaleiteDeliveryDayInputs.forEach(input=>{
 input.addEventListener("change",updateVivaleiteQuality);
});
vivaleiteTodayBtn.addEventListener("click",()=>{
 vivaleiteActive=true;
 vivaleiteTodayOnly=!vivaleiteTodayOnly;
 vivaleiteTodayBtn.classList.toggle("active",vivaleiteTodayOnly);
 vivaleiteTodayBtn.textContent=vivaleiteTodayOnly
  ?"🥛 Mostrando VIVALEITE hoje"
  :"🥛 VIVALEITE hoje";
 renderMap();
});

if(mobileMapInteractBtn){
 mobileMapInteractBtn.addEventListener("click",toggleMobileMapInteraction);
}
if(mobileMapExpandBtn){
 mobileMapExpandBtn.addEventListener("click",toggleMobileMapExpansion);
}
if(mobileFab){
 mobileFab.addEventListener("click",openRecordTypeModal);
}
if(closeMobileMoreSheet){
 closeMobileMoreSheet.addEventListener("click",closeMobileMoreSheetPanel);
}
if(mobileSheetBackdrop){
 mobileSheetBackdrop.addEventListener("click",closeMobileMoreSheetPanel);
}
if(mobileBottomNav) mobileBottomNav.addEventListener("click",event=>{
 const button=event.target.closest("button[data-mobile-action]");if(!button)return;
 const action=button.dataset.mobileAction;
 if(action==="map")goToMobileMap();
 if(action==="search")focusMobileSearch();
 if(action==="briefing")openGabinetePanel();
 if(action==="favorites")openFavoritesPanel();
 if(action==="more")openMobileMoreSheet();
});
if(mobileMoreSheet) mobileMoreSheet.addEventListener("click",event=>{
 const button=event.target.closest("button[data-sheet-action]");if(!button)return;
 const action=button.dataset.sheetAction;
 if(action==="explore"){closeMobileMoreSheetPanel();openExplorePanel()}
 if(action==="today"){closeMobileMoreSheetPanel();openDailyReportPanel()}
 if(action==="agenda"){closeMobileMoreSheetPanel();openCalendar()}
 if(action==="visit"){closeMobileMoreSheetPanel();openVisitFolderPanel()}
 if(action==="central"){closeMobileMoreSheetPanel();openDataCenterPanel()}
 if(action==="location")useCurrentLocation()
});
window.addEventListener("resize",initializeMobileExperience);
initializeMobileExperience();

[
 vivaleiteNameInput,vivaleiteStatusInput,
 vivaleiteStartInput,vivaleiteEndInput,vivaleiteRegionInput,
 vivaleiteBairroInput,vivaleiteAddressInput,vivaleiteLatitudeInput,
 vivaleiteLongitudeInput,vivaleitePublicDescriptionInput,
 vivaleiteLocalResponsibleInput,vivaleiteSubstituteInput,
 vivaleitePhoneInput,vivaleiteEmailInput,vivaleiteChildrenInput,
 vivaleiteElderlyInput,vivaleiteCapacityInput,vivaleiteVacanciesInput,
 vivaleiteDeliveryStatusInput,vivaleiteDeliveryReasonInput,
 vivaleiteInternalNotesInput
].forEach(control=>{
 control.addEventListener("input",updateVivaleiteQuality);
 control.addEventListener("change",updateVivaleiteQuality);
});
closeFeiraFormBtn.addEventListener("click",closeFeiraForm);
cancelFeiraFormBtn.addEventListener("click",closeFeiraForm);
feiraFormModal.addEventListener("click",event=>{if(event.target===feiraFormModal)closeFeiraForm()});
feiraForm.addEventListener("submit",event=>{if(event.submitter===saveFeiraFormBtn)saveAndCreateAnotherFeira=false;saveFeiraForm(event)});
saveAndNewFeiraFormBtn.addEventListener("click",()=>{saveAndCreateAnotherFeira=true;feiraForm.requestSubmit()});
feiraBairroInput.addEventListener("change",()=>{inferFeiraRegion();updateFeiraFormQuality()});
[feiraNameInput,feiraDayInput,feiraStatusInput,feiraStartInput,feiraEndInput,feiraRegionInput,feiraBairroInput,feiraAddressInput,feiraTrecho1Input,feiraTrecho2Input,feiraTrecho3Input,feiraTrecho4Input,feiraLatitudeInput,feiraLongitudeInput,feiraNotesInput].forEach(control=>{control.addEventListener("input",updateFeiraFormQuality);control.addEventListener("change",updateFeiraFormQuality)});

favoritesBtn.addEventListener("click",openFavoritesPanel);
closeFavoritesBtn.addEventListener("click",closeFavoritesPanel);
favoritesBackdrop.addEventListener("click",closeFavoritesPanel);
visitFolderBtn.addEventListener("click",openVisitFolderPanel);
closeVisitFolderBtn.addEventListener("click",closeVisitFolderPanel);
visitFolderBackdrop.addEventListener("click",closeVisitFolderPanel);
clearVisitFolderBtn.addEventListener("click",()=>{visitFolderKeys=[];persistLibraryState();renderVisitFolderPanel();renderDataCenterList()});
sendVisitToGabineteBtn.addEventListener("click",sendVisitFolderToGabinete);
persistLibraryState();
...
gabinetePreviewBtn.addEventListener("click",event=>{
  event.preventDefault();

  try{
    buildGabinetePreview();
  }catch(error){
    console.error("Erro ao abrir a prévia do briefing:",error);
    window.alert(
      "Não foi possível abrir a prévia do briefing.\n\n"+
      (error && error.message ? error.message : "Erro desconhecido.")
    );
  }
});
...
if(exploreVivaleiteToggle){
 exploreVivaleiteToggle.addEventListener("change",()=>{
  vivaleiteActive=exploreVivaleiteToggle.checked;
  renderMap();
  updateExplorePanel();
 });
}
...
window.addEventListener("error",event=>{
 console.error("Erro de inicialização:",event.error||event.message);
 if(statusBox && statusBox.textContent.includes("Carregando")){
  statusBox.textContent="Erro de inicialização: "+(event.message||"verifique o Console");
 }
});
...
window.addEventListener("unhandledrejection",event=>{
 ...
 if(statusBox && statusBox.textContent.includes("Carregando")){
  ...
  statusBox.textContent="Erro ao carregar os dados: "+message;
 }
});
