console.log("GeoDiadema 2.7.2 carregado");
...
const eventRegionSuggestions=document.getElementById("eventRegionSuggestions");
const eventBairroSuggestions=document.getElementById("eventBairroSuggestions");
...
function getDataCenterPendingFields(item){
 const p=[];
 if(!String(item.evento||"").trim())p.push("nome");
 if(!String(item.data||"").trim())p.push("data");
 if(!String(item.horaInicio||"").trim())p.push("hora inicial");
 if(!String(item.horaFim||"").trim())p.push("hora final");
 if(!String(item.bairro||"").trim())p.push("bairro");
 if(!String(getItemEffectiveRegion(item)||"").trim())p.push("região");
 if(!String(item.endereco||item.local||"").trim())p.push("endereço/local");
 if(!String(item.secretariaResponsavel||item.secretaria||"").trim())p.push("secretaria");
 if(parseCoordinate(item.latitude)===null||parseCoordinate(item.longitude)===null)p.push("coordenadas");
 return p;
}
function getDataCenterFeiras(){return allEvents.filter(isFeiraGabineteItem)}
function calculateDataQuality(){
 const r=allEvents.filter(x=>!isFeiraGabineteItem(x)), total=r.length*9;
 if(!total)return{percent:100,pendingRecords:0,missingFields:0};
 const missing=r.reduce((s,x)=>s+getDataCenterPendingFields(x).length,0);
 return{percent:Math.max(0,Math.round((total-missing)/total*100)),pendingRecords:r.filter(x=>getDataCenterPendingFields(x).length).length,missingFields:missing};
}
...
function updateDataCenterIndicators(){
 ...
 dataQualitySummary.textContent=`${e.length} evento(s) analisado(s) • ${q.pendingRecords} registro(s) com pendência • ${q.missingFields} campo(s) incompleto(s)`;
}
...
function renderDataCenterList(){
 const s=normalize(dataCenterSearch.value),r=normalize(dataCenterRegionFilter.value),st=normalize(dataCenterStatusFilter.value);
 const items=getDataCenterModuleItems().filter(x=>{
  const hay=normalize([getDataCenterRecordName(x),x.secretariaResponsavel,x.bairro,getItemEffectiveRegion(x),x.local,x.endereco,x.status].filter(Boolean).join(" "));
  return(!s||hay.includes(s))&&(!r||normalize(getItemEffectiveRegion(x))===r)&&(!st||normalize(x.status)===st)
 }).sort((a,b)=>getDataCenterRecordName(a).localeCompare(getDataCenterRecordName(b),"pt-BR"));
 dataCenterList.innerHTML="";
 if(!items.length){dataCenterList.innerHTML='<div class="gabinete-empty">Nenhum registro encontrado.</div>';return}
 items.slice(0,250).forEach(x=>{
  const p=dataCenterActiveModule==="pending"?getDataCenterPendingFields(x):[];
  const card=document.createElement("article");card.className="data-center-record"+(p.length?" data-center-pending":"");
  card.innerHTML=`<div><h3>${escapeHtml(getDataCenterRecordName(x))}</h3><p><strong>Data:</strong> ${escapeHtml(x.data||"-")} • <strong>Horário:</strong> ${escapeHtml(x.horaInicio||"-")} às ${escapeHtml(x.horaFim||"-")}</p><p><strong>Secretaria:</strong> ${escapeHtml(x.secretariaResponsavel||x.secretaria||"-")} • <strong>Território:</strong> ${escapeHtml(x.bairro||"-")} / ${escapeHtml(getItemEffectiveRegion(x)||"-")}</p><p><strong>Local:</strong> ${escapeHtml(x.local||x.endereco||"-")}</p>${p.length?`<p class="data-center-pending-fields">Pendências: ${escapeHtml(p.join(", "))}</p>`:""}</div><div class="data-center-record-actions">
<button class="edit-record-btn" type="button">${p.length?"✏️ Corrigir agora":"✏️ Editar"}</button>
<button class="duplicate-record-btn" type="button">📄 Duplicar</button>
  ...
  editButton.addEventListener("click",()=>openEventForm("edit",x));
  duplicateButton.addEventListener("click",()=>openEventForm("duplicate",x));
 });
}
...
function fillEventFormSuggestions(){
  ...
  eventRegionSuggestions.innerHTML=regions
    .map(value=>`<option value="${escapeHtml(value)}"></option>`)
    .join("");

  eventBairroSuggestions.innerHTML=bairros
    .map(value=>`<option value="${escapeHtml(value)}"></option>`)
    .join("");
}
...
function openEventForm(mode,record=null){
...
newDataRecordBtn.addEventListener("click",()=>openEventForm("new"));
...
document.querySelectorAll(".data-center-module").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll(".data-center-module").forEach(x=>x.classList.remove("active"));b.classList.add("active");dataCenterActiveModule=b.dataset.module||"events";renderDataCenterList()}));
[dataCenterSearch,dataCenterRegionFilter,dataCenterStatusFilter].forEach(c=>{c.addEventListener("input",renderDataCenterList);c.addEventListener("change",renderDataCenterList)});
