console.log("GeoDiadema 2.2 carregado");
...
function buildGabinetePreview() {
  ...
  gabinetePreviewDocument.innerHTML = `
    <header class="briefing-cover">
      <div class="briefing-cover-brand">
        <div>
          <h1>GeoDiadema</h1>
          <p>Trabalhando por Diadema</p>
        </div>
        <div class="briefing-code">${escapeHtml(briefingCode)}</div>
      </div>

      <div class="briefing-cover-label">AGENDA EXECUTIVA</div>
      <h2>${escapeHtml(title)}</h2>

      <div class="briefing-cover-info">
        <div>
          <span>Data</span>
        ...
        <div>
          <span>Período</span>
          <strong>${escapeHtml(gabinetePeriod.options[gabinetePeriod.selectedIndex].textContent)}</strong>
        </div>
        <div>
          <span>Horário previsto</span>
          <strong>${escapeHtml(agendaRange)}</strong>
        </div>
      </div>

      ${objective ? `
        <div class="briefing-objective">
          <span>Objetivo da agenda</span>
    ...
    <section class="briefing-summary-section">
      ...
      <div class="briefing-summary-grid">
        ...
        <div>
          <strong>${hortaCount}</strong>
          <span>Hortas</span>
        </div>
        <div>
          <strong>${escapeHtml(agendaRange)}</strong>
          <span>Agenda prevista</span>
        </div>
      </div>
    </section>

    <section class="briefing-agenda-section">
      <div class="briefing-section-title">
    ...
    <footer class="briefing-footer">
      <strong>Documento de apoio ao gabinete</strong><br>
      Gerado pelo GeoDiadema em ${new Date().toLocaleString("pt-BR")}
    </footer>
  `;
